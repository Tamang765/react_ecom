...
const Subcategorylist = () => {
  ...
  const deleteSubCategory = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://127.0.0.1:8000/product/updateDeleteSubCategory/${id}/`);
        ...
        console.log("Delete Sub Category failed", e.message);
          ...
          <tbody>
            {
              data.map((items, i) => (
                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={i}>
                  ...
                  <td className="px-6 py-4">{items.category_title}</td>
                  <td className="px-6 py-4">
                    <Link to={`/etrendadmin/editsub/${items.id}`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</Link> &nbsp; &nbsp;
                    <Link to="#" onClick={() => deleteSubCategory(items.id)} className="font-medium text-red-600 dark:text-red-500 hover:underline">Delete</Link>
                  </td>
                </tr>
              ))
            }
